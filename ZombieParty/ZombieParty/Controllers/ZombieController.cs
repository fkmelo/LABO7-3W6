...
namespace ZombieParty.Controllers
{
    public class ZombieController : Controller
    {
        ...
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.Include(z => z.ZombieType).ToList();
            List<Zombie> zombiesTrie = zombiesList.OrderBy(z => z.Name).ToList();

            return View(zombiesTrie);
        }

        public IActionResult StrongestZombies()
        {
            List<Zombie> zombiesList = _baseDonnees.Zombies.Include(z => z.ZombieType).ToList();
            List<Zombie> zombiesTrie = zombiesList.Where(z => z.Point >= 8).ToList();
